 ...
 * This allows database configuration to be managed via environment variables
 * instead of being hardcoded in this file.
 */
foreach ([__DIR__ . '/../../../.env', __DIR__ . '/../../../.env.local'] as $_env_path) {
  if (file_exists($_env_path)) {
    $env_file = file_get_contents($_env_path);
    foreach (preg_split('/\n/', $env_file) as $line) {
...
$databases['default']['default'] = [
    'driver' => 'mysql',
    'host' => $_ENV['DB_D10_HOST'],
    'database' => $_ENV['DB_D10_NAME'],
    'username' => $_ENV['DB_D10_USER'],
    'password' => $_ENV['DB_D10_PASSWORD'],
    'port' => $_ENV['DB_D10_PORT'],
  ];
/**
 * Symfony database connection
 */
$databases['symfony']['default'] = [
  'driver' => 'mysql',
  'host' => $_ENV['DB_SYMFONY_HOST'] ?: $_ENV['DB_D10_HOST'],
  'database' => $_ENV['DB_SYMFONY_NAME'],
  'username' => $_ENV['DB_SYMFONY_USER'] ?: $_ENV['DB_D10_USER'],
  'password' => $_ENV['DB_SYMFONY_PASSWORD'] ?: $_ENV['DB_D10_PASSWORD'],
  'port' => $_ENV['DB_SYMFONY_PORT'] ?: $_ENV['DB_D10_PORT'],
  'prefix' => '',
  'collation' => 'utf8mb4_general_ci',
];
...
/**
 * Trusted Host Settings - SECURITY
 * 
 * Protects against HTTP HOST Header attacks by restricting which hostnames
 * are considered valid. Only requests with a matching Host header will be
 * processed.
 * 
 * Configure TRUSTED_HOST_PATTERNS in .env file (comma-separated):
 * TRUSTED_HOST_PATTERNS=localhost,normandie.local
 */
if (!empty($_ENV['TRUSTED_HOST_PATTERNS'])) {
  $hosts = array_map('trim', explode(',', $_ENV['TRUSTED_HOST_PATTERNS']));
  $settings['trusted_host_patterns'] = array_map(function($host) {
    // Convert simple hostname to regex pattern
    return '^' . str_replace('.', '\.', $host) . '$';
  }, $hosts);
}
else {
  // Fallback for safety (minimal configuration)
  $settings['trusted_host_patterns'] = [
    '^localhost$',
    '^127\.0\.0\.1$',
  ];
...
/**
 * Connection to Drupal 7 source database for migration.
 * 
 * This configuration allows Drush to access the source Drupal 7 database
 * for the migration process using the migrate module.
 * All values are read from environment variables in .env file.
 */
$databases['migrate']['default'] = [
  'driver' => 'mysql',
  'host' => $_ENV['DB_D7_HOST'],
  'database' => $_ENV['DB_D7_NAME'],
  'username' => $_ENV['DB_D7_USER'],
  'password' => $_ENV['DB_D7_PASSWORD'],
  'port' => $_ENV['DB_D7_PORT'],
];

/**
 * Connection to Drupal 7 source database (d7_source key for migration scripts).
 * 
 * This key 'd7_source' is used by migration scripts and Drush commands
 * to access the source Drupal 7 database.
 * All values are read from environment variables in .env file.
 */
$databases['d7_source']['default'] = [
  'driver' => 'mysql',
  'host' => $_ENV['DB_D7_HOST'],
  'database' => $_ENV['DB_D7_NAME'],
  'username' => $_ENV['DB_D7_USER'],
 ...
 * 
 * Default settings for the migration system.
 * Values are read from environment variables in .env file when available.
 */
$settings['migrate_source_version'] = '7';
$settings['migrate_source_connection'] = 'migrate';
$settings['migrate_file_public_path'] = $_ENV['MIGRATE_FILE_PUBLIC_PATH'];
...
$settings['config_sync_directory'] = 'sites/default/files/config_qb87jq4Rp0umOmeN_woCP78pWFcmvSm5911qk0ZitquIOuni4YTjxZEMgLD205O_3v2Inrfnvw/sync';

/**
 * Environment-Specific Settings
 * 
 * Uses DRUPAL_ENV environment variable to control all environment-specific behavior.
 * Set DRUPAL_ENV in your .env file:
 * - DRUPAL_ENV=dev for development (disables caches, shows errors)
 * - DRUPAL_ENV=prod for production (enables caches, hides errors)
 * 
 * Development mode (.env: DRUPAL_ENV=dev):
 * - Disables all caches for immediate feedback
 * - Shows all errors and warnings
 * - Disables CSS/JS aggregation for easier debugging
 * - Skips file permissions hardening
 * - Loads development.services.yml for Twig debugging
 * 
 * Production mode (.env: DRUPAL_ENV=prod or unset):
 * - Enables all caches for performance
 * - Hides errors from visitors
 * - Enables CSS/JS aggregation
 * - Disables update checks
 */
$drupal_env = $_ENV['DRUPAL_ENV'] ?? 'prod';

/**
 * External URLs for Connexion links (beneficiaire / partenaire).
 * Configured per environment in .env files.
 */
$settings['url_beneficiaire'] = $_ENV['URL_BENEFICIAIRE'] ?? '';
$settings['url_partenaire'] = $_ENV['URL_PARTENAIRE'] ?? '';

if ($drupal_env === 'dev') {
  // Development: Disable all caches
  ...
  // Development: Disable CSS/JS aggregation for easier debugging
  $config['system.performance']['css']['preprocess'] = FALSE;
  $config['system.performance']['js']['preprocess'] = FALSE;
  
  // Development: Skip file permissions hardening
  $settings['skip_permissions_hardening'] = TRUE;
  
  // Development: Load development services (Twig debugging, etc.)
  $settings['container_yamls'][] = DRUPAL_ROOT . '/sites/development.services.yml';
}
else {
  // Production: Enable CSS/JS aggregation for performance
  $config['system.performance']['css']['preprocess'] = TRUE;
  $config['system.performance']['js']['preprocess'] = TRUE;
  
  // Production: Enable page cache
  $config['system.performance']['cache']['page']['max_age'] = 3600;
  
  // Production: Hide errors from visitors
  $config['system.logging']['error_level'] = 'hide';
  
  // Production: Disable update checks
  $config['update.settings']['check']['disabled_extensions'] = TRUE;
}

/**
 * Mail transport configuration (SMTP/Mailhog)
 * 
 * Configures mail transport using MAILER_DSN environment variable.
 * This allows different mail configurations per environment:
 * 
 * - Local, Dev: MAILER_DSN=smtp://localhost:1025 (Mailhog)
 * - Prod: MAILER_DSN=smtp://smtp.gmail.com:587?encryption=tls&username=user@gmail.com&password=pass
 * 
 */
if (!empty($_ENV['MAILER_DSN'])) {
  // Parse the MAILER_DSN to extract SMTP settings
  $mailer_dsn = $_ENV['MAILER_DSN'];
  
  // Parse DSN format: smtp://host:port or smtp://user:pass@host:port
  if (preg_match('#^smtp://(?:([^:@]+):([^@]+)@)?([^:]+):(\d+)(?:\?(.*))?$#', $mailer_dsn, $matches)) {
    $smtp_username = $matches[1] ?? '';
    $smtp_password = $matches[2] ?? '';
    $smtp_host = $matches[3];
    $smtp_port = $matches[4];
    $smtp_params = $matches[5] ?? '';
    
    // Parse additional parameters (encryption, ssl_no_verify, etc.)
    $smtp_encryption    = '';
    $smtp_ssl_no_verify = FALSE;
    if (!empty($smtp_params)) {
      parse_str($smtp_params, $params);
      $smtp_encryption    = $params['encryption'] ?? '';
      // ssl_no_verify=1 : désactive la vérification SSL (certificat auto-signé)
      $smtp_ssl_no_verify = !empty($params['ssl_no_verify']);
      if (empty($smtp_username) && !empty($params['username'])) {
        $smtp_username = $params['username'];
      }
      if (empty($smtp_password) && !empty($params['password'])) {
        $smtp_password = $params['password'];
      }
    }

    // Port 25 = relais ouvert (sans auth, sans TLS).
    // - Force protocol=standard pour éviter SSL/TLS
    if ((int) $smtp_port === 25 && $smtp_encryption === '') {
      $smtp_encryption    = 'standard';
      $smtp_ssl_no_verify = TRUE;
    }
    ...
    // Store SMTP settings for use by mail plugins/modules
    $config['smtp.settings']['smtp_host']     = $smtp_host;
    $config['smtp.settings']['smtp_port']     = $smtp_port;
    $config['smtp.settings']['smtp_username'] = $smtp_username;
    $config['smtp.settings']['smtp_password'] = $smtp_password;
    $config['smtp.settings']['smtp_protocol'] = $smtp_encryption ?: 'standard';
    $config['smtp.settings']['smtp_on']       = TRUE;

    // Désactive SMTPAutoTLS quand aucun chiffrement n'est demandé.
    $config['smtp.settings']['smtp_autotls'] = !empty($smtp_encryption) && $smtp_encryption !== 'standard';

    // Désactive la vérification du certificat SSL si demandé
    // (certificat auto-signé, relais interne, port 25).
    if ($smtp_ssl_no_verify) {
      $config['smtp.settings']['smtp_ssl_verify_peer']       = FALSE;
      $config['smtp.settings']['smtp_ssl_verify_peer_name']  = FALSE;
      $config['smtp.settings']['smtp_ssl_allow_self_signed'] = TRUE;
      // Options passées à stream_context_create() via PHPMailer->SMTPOptions
      $config['smtp.settings']['smtp_ssl_context_options'] = [
        'ssl' => [
          'verify_peer'       => FALSE,
    ...
    // Set SMTP as the default mail system
